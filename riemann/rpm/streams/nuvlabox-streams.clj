...
(defn- name-id-state-last-online
       ...
       {:name         (keyword (get-in connector [:connector :href]))
        :id           (:id connector)
        :state        (if (= :ok (-> connector :schema-org:state keyword)) :ok :nok)
        :last-online  (:schema-org:last-online connector)})
...
(defn- fetch-topology
       ...
       (->>  (try-get-connectors)
             (filter #(contains? % :schema-org:state)) 
...
(defn- send-state
       ...
       (info "State was sent for " name ": " id-state-last-online))
